...
# 昇格の最低間隔。2026-09-21 に 24h15m → 23h へ下げた。
# cronを毎日20:00 JST固定にしたので、連続する実行の間隔は24時間ちょうどになる。
# 24h15m のままだと毎回このガードに掛かって1日おきにしか昇格しない。
#
# 🚨 この間隔は「Zennに通るか」の予測には使えない。24h15m以上あけても
# 09-18 03:03（25h01m）と 09-21 06:02（25h00m）は弾かれた。効いていたのは
# 昇格の間隔ではなく、前の記事が公開されてからの経過時間のほうだった
# （→ .github/workflows/drip.yml のコメント）。ここは二重昇格を防ぐ歯止めとして残す
MIN_INTERVAL = datetime.timedelta(hours=23)
