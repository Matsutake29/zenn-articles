...
GitHub Actions（`.github/workflows/drip.yml`）が **09:00〜12:00 JST の毎時**に動き、
条件を満たした最初のtickで1本昇格させて、その日の **18:00 JST 公開** で予約する。
公開まで最低6時間の猶予があるので、弾かれても当日中に手を打てる。
...
昇格時刻は毎日15分ずつ後ろへずれ、12:00を越えた日は翌朝に回る（12日に1回程度、1日空く）。
**公開時刻は `published_at` で18:00に固定される**ので、読者から見た公開時刻は動かない。
ウィンドウを広げれば空く頻度は下がるが、昇格が遅くなるほど公開18:00までの猶予が減る。
